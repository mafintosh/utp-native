...
module.exports = class Connection extends Duplex {
  ...
  _writev (datas, cb) {
    ...
    const drained = binding.utp_napi_connection_writev(this._handle, bufs)

    if (drained) cb(null)
    else this._writing = [cb, bufs]
  }
  ...
  _resolveAndConnect (port, host) {
    dns.lookup(host, { family: 4 }, (err, ip) => {
      ...
      if (!ip) return this.destroy(new Error('Could not resolve ' + host))
  ...
  _open (cb) {
    if (this._connected) cb(null)
    else this.once('connect', cb)
  }
